...
export function validateNodeConfig(
  ...
  switch (nodeType) {
    case "MESSAGE":
      return validateMessageConfig(config as MessageNodeConfig);
    case "PROMPT":
...
export function validateMessageConfig(
  ...
  if (!isNonEmptyString(config.text)) {
    errors.push({
      field: "text",
      message: getRequiredFieldError("Message text"),
    });
  }
  ...
  if (config.text) {
    ...
    const variableValidation = validateTemplateVariables(config.text, "MESSAGE");
  ...
  routes?.forEach((route, index) => {
    if (!route.condition || route.condition.trim() === "") {
      ...
      if (!isAutoGeneratedFallback && !isValidCondition(nodeType, route.condition, nodeConfig)) {
        ...
        if (["MENU", "API_ACTION", "PROMPT", "MESSAGE"].includes(nodeType)) {
          errors.push({
            field: `routes[${index}].condition`,
            message:
