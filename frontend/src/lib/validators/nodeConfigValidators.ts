...
export function validateRoutes(
  ...
  // Routes are optional - 0 routes means the node is terminal
  // No validation error for empty routes
  ...
  routes?.forEach((route, index) => {
    if (!route.condition || route.condition.trim() === "") {
      ...
      // For MENU and API_ACTION, allow "true" as a default fallback route
  ...
  // These are default fallback routes and shouldn't count against the max
