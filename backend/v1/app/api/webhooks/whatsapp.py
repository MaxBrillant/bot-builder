...
from app.core.engine import ConversationOrchestrator, get_http_client, MessageCallback
...
def create_whatsapp_callback(
    integration_manager: IntegrationManager,
    bot_id: UUID,
    channel_user_id: str
) -> MessageCallback:
    """
    Create a callback that sends messages to WhatsApp in real-time.

    Each message is sent immediately as it's generated by the engine,
    providing real-time message delivery to the user.
    """
    async def callback(message: str, is_final: bool) -> None:
        if not message:
...
async def receive_whatsapp_message(
    ...
    # Process through conversation engine with real-time WhatsApp delivery
    try:
        # Create callback that sends messages to WhatsApp as they're generated
        whatsapp_callback = create_whatsapp_callback(
            integration_manager,
            bot_id,
            normalized_message["channel_user_id"]
        )
        ...
        result = await orchestrator.process_message(
            ...
            message_callback=whatsapp_callback  # Messages sent in real-time via callback
        )
